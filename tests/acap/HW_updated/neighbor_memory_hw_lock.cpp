...
template <typename AIE, int X, int Y> struct prog : acap::aie::tile<AIE, X, Y> {
  ...
  bool prerun() {
    ...
    t::mem_write(acap::hw::tile_mem_begin_offset, written_data.back().x);
    /// writes to tile_memory::y
    t::mem_write(acap::hw::tile_mem_begin_offset + 4, written_data.back().y);
    /// writes to tile_memory::id
    t::mem_write(acap::hw::tile_mem_begin_offset + 8, written_data.back().id);
