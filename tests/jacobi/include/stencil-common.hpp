...
class local_info2D {
private:
  static const bool check1 = _nb_tab * sizeof(T) * (_nbi_wg0 + d0 + 1) * (_nbi_wg1 + d1) <= CL_DEVICE_LOCAL_MEM_SIZE;
  static const bool check2 = _nb_tab * sizeof(T) * (_nbi_wg0 + d0) * (_nbi_wg1 + d1 + 1) <= CL_DEVICE_LOCAL_MEM_SIZE;
  static const bool check3 = _nb_tab * sizeof(T) * (_nbi_wg0 + d0 + 1) * (_nbi_wg1 + d1 + 1) <= CL_DEVICE_LOCAL_MEM_SIZE;

  static const bool check4 = (_nbi_wg0 + 1) <= CL_DEVICE_CONST_WORK_GROUP_SIZE0;
  static const bool check5 = (_nbi_wg1 + 1) <= CL_DEVICE_CONST_WORK_GROUP_SIZE1;
...
public:
  static_assert((d0+1 <= CL_DEVICE_CONST_WORK_GROUP_SIZE0) || (d1+1 <= CL_DEVICE_CONST_WORK_GROUP_SIZE1), "Stencil too large.");
